...
for i in range(n_mpc):
    for j in range(params.ratio):
        ii = i * params.ratio + j
        Y_wb[ii+1,:] = wbc.integrateCoMLateralState(Y_mpc[i,:], U_y_mpc[i], (j+1)*params.dt)
...
count = 0
for i in range(n_wb):
    wbc.set_time(count * params.dt, T_pred[0])
    count += 1
...
# NB : to test with T_pred you have to remove t_current in alpha (LIPM to WBD)
