...
y_real = np.empty((n_steps+1, 2)) * nan
...
x_dot_com = np.array([params.x_hat_0[1], params.y_hat_0[1], 0.])

t = 0
q, v = ik.q, ik.compute_state_velocity(x_LF_0[:,1], x_RF_0[:,1], x_dot_com)
hs.t_HS = round(t_init / params.dt) * params.dt

# Try with an external robot model
robot = RobotWrapper.BuildFromURDF(params.urdf, [params.path], pin.JointModelFreeFlyer())
rmodel = robot.model
rdata = rmodel.createData()

com_pos_pino = np.empty((n_steps+1, 3)) * nan
com_vel_pino = np.empty((n_steps+1, 3)) * nan
com_pos_pino[0,:], com_vel_pino[0,:] = robot.com(q, v)
x_real[0,0], x_real[0,1] = com_pos_pino[0,0], com_vel_pino[0,0]
y_real[0,0], y_real[0,1] = com_pos_pino[0,1], com_vel_pino[0,1]
...
for i in range(n_steps):
    # From the real values get the hybrid system state
    ...
    if i % params.ratio == 0:
        ...
        z_y[j] = mpc.Z_y
        j += 1
    # Perform the integration (flow dynamics, just to increase the time counter)
    _ = hs.flow(x[i,:], u_x[i])
    ...
    com_pos_ref[i, :] = np.array([x_real[i, 0], y_real[i, 0], params.h_com])
    com_vel_ref[i, :] = np.array([x_real[i, 1], y_real[i, 1], 0])
    ...
    # Retrieve the next CoM position and velocity from the robot (pinocchio)
    com_pos_pino[i+1,:], com_vel_pino[i+1,:] = robot.com(q, v)

    # Assign to correct arrays
    x_real[i+1,0], x_real[i+1,1] = com_pos_pino[i+1,0], com_vel_pino[i+1,0]
    y_real[i+1,0], y_real[i+1,1] = com_pos_pino[i+1,1], com_vel_pino[i+1,1]
...
if PLOT_COM:
    ...
    ax[0].plot(t[:n_steps], com_pos_pino[:n_steps,0], color='turquoise', linestyle='dashed', label='$x_{pino}$')
    ...
    ax[1].plot(t[:n_steps], com_pos_pino[:n_steps,1], color='turquoise', linestyle='dashed', label='$y_{pino}$')
    ...
    ax[2].plot(t[:n_steps], com_pos_pino[:n_steps,2], color='turquoise', linestyle='dashed', label='$z_{pino}$')
    ...
    ax[0].plot(t[:n_steps], com_vel_pino[:n_steps,0], color='turquoise', linestyle='dashed', label='$\dot x_{pino}$')
    ...
    ax[1].plot(t[:n_steps], com_vel_pino[:n_steps,1], color='turquoise', linestyle='dashed', label='$\dot y_{pino}$')
    ...
    ax[2].plot(t[:n_steps], com_vel_pino[:n_steps,2], color='turquoise', linestyle='dashed', label='$\dot z_{pino}$')
